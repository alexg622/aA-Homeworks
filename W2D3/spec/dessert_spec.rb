...
describe Dessert do
  ...
  let(:dessert) { Dessert.new("good", 10, chef)}
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect{Dessert.new("bad", "bad", chef)}.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#eat" do
    it "subtracts an amount from the quantity" do
      ...
      dessert.eat(5)
      expect(dessert.quantity).to be < quantity
    end
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      ...
      expect(dessert.serve).to eq("Chef chef the Great Baker has made 10 goods!")
    end
  end
